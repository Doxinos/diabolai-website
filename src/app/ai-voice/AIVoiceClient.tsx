...
/* ------------------------------------------------------------------ */
const VoiceHero = ({ onBookCall }: { onBookCall: () => void }) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-0 items-start">
          {steps.map((step, i) => (
            <div key={step.num} className="hiw-step flex md:flex-col items-start gap-5 md:px-6 relative">
              ...
              {i < steps.length - 1 && (
                <span className="hidden md:block absolute top-5 -right-1 text-[rgba(17,17,17,0.20)] text-lg">&rarr;</span>
              )}
    ...
    <section ref={ref} data-theme="dark" className="w-full bg-[#0A2843] px-6 py-32 md:px-12">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="relative rounded-2xl bg-[#060e1a] border border-[rgba(255,255,255,0.10)] p-6 md:p-8">
          ...
          <div className="absolute top-4 right-4 md:top-6 md:right-6 flex items-center gap-2">
            <span className="relative flex h-2.5 w-2.5">
              <span className="absolute inline-flex h-full w-full rounded-full bg-[#FF4F30] opacity-75 animate-ping" />
          ...
          <div className="font-mono text-[12px] md:text-[13px] leading-relaxed text-[rgba(255,255,255,0.80)] min-h-[300px] max-h-[500px] overflow-y-auto pr-4">
            ...
            {!showConfirm && started && (
              <span className="inline-block w-2 h-4 bg-[#FF4F30] ml-1 animate-blink" />
            )}
          </div>
        ...
        <p className="mt-6 text-center font-mono text-[11px] uppercase tracking-[0.14em] text-[rgba(255,255,255,0.35)]">
          Built with Vapi &middot; ElevenLabs Voice &middot; n8n Automation
        </p>
      </div>
    </section>
  );
...
/* ------------------------------------------------------------------ */
/* WHAT YOU GET — 6 outcome cards in 2x3 grid                          */
/* ------------------------------------------------------------------ */
const WhatYouGet = () => {
  const ref = useRef<HTMLDivElement>(null);
...
/* ------------------------------------------------------------------ */
const PilotOffer = ({ onBookCall }: { onBookCall: () => void }) => {
  const ref = useRef<HTMLDivElement>(null);
    ...
    <section ref={ref} className="w-full bg-[#DCDBD3] px-6 py-32 md:px-12">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="pilot-card rounded-2xl bg-[#111111] p-8 md:p-10 text-[#DCDBD3] max-w-3xl">
          ...
          {/* Delivery */}
          <div className="mb-8">
            <span className="text-[clamp(28px,4vw,32px)] font-extrabold text-white">Live in 3 weeks</span>
            <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-[rgba(255,255,255,0.40)] mt-1">
              Custom quote &middot; No monthly lock-in
            </p>
          </div>
          ...
          <button
            onClick={onBookCall}
            className="btn-magnetic rounded-full bg-[#FF4F30] px-8 py-4 text-sm font-bold text-white hover:bg-[#E64528]"
          >
            Start Your Pilot
          </button>
        </div>
...
/* ------------------------------------------------------------------ */
const VoiceFinalCTA = ({ onBookCall }: { onBookCall: () => void }) => {
  const ref = useRef<HTMLDivElement>(null);
    ...
    <section ref={ref} data-theme="dark" className="w-full bg-[#0A2843] px-6 py-32 md:px-12">
      <div className="fcta-content mx-auto max-w-xl text-center">
        ...
        <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.14em] text-[rgba(255,255,255,0.35)]">
          No commitment &middot; 30 minutes &middot; Free
        </p>
      </div>
    </section>
  );
  ...
  return (
    <>
      ...
      <main className="relative w-full selection:bg-[#FF4F30] selection:text-white">
        <Navbar />
        <VoiceHero onBookCall={handleBookCall} />
        <Problem />
        <HowItWorks />
        <LiveDemo />
        <WhatYouGet />
        <FullBleedBreak />
        <PilotOffer onBookCall={handleBookCall} />
        <Objections />
